			...
			<c:choose>
				<c:when test="<%= PortalUtil.isLayoutFriendliable(selLayout) %>">
					<aui:field-wrapper cssClass="input-append input-flex-add-on input-prepend" helpMessage='<%= LanguageUtil.format(pageContext, "for-example-x", "<em>/news</em>") %>' label="friendly-url" name="friendlyURL">
						...
						<span class="add-on" id="<portlet:namespace />urlBase">
						<liferay-ui:message key="<%= StringUtil.shorten(friendlyURLBase.toString(), 40) %>" /></span>
						...
						<label id="<portlet:namespace />urlBase"> URL: <br>
							<liferay-ui:message key="<%= friendlyURLBase.toString() %>" />
							<liferay-ui:message key="<%= (selLayout != null) ? selLayout.getFriendlyURL() : StringPool.BLANK %>" />
						</label>
